...
local env_file = io.open(base_path .. "/lua/environment.lua", "r")
if env_file~=nil
then
	...
	local example_env = io.open(base_path .. "/lua/environment.lua.example", "r")
	local example_env_contents = example_env:read("*a")
	example_env:close()

	-- Paste into a new environment file
	env_file = io.open(base_path .. "/lua/environment.lua", "w")
